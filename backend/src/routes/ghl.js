...
router.post('/webhook', async (req, res) => {
  try {
    ...
    if (eventType === 'OutboundMessage' || eventType === 'SMS' || payload.messageType === 'SMS') {
      ...
      // Skip messages that contain WhatsApp delivery metadata
      // GHL Conversation Provider appends "☑️from other device" to message body
      // when it detects delivery, then re-fires the webhook causing an infinite loop
      if (messageContent && messageContent.includes('from other device')) {
        logger.info('Skipping GHL message with delivery metadata:', {
          phone: phoneNumber, contentPreview: messageContent.substring(0, 80)
        });
        return res.status(200).json({ success: true, message: 'Delivery metadata skipped' });
      }
